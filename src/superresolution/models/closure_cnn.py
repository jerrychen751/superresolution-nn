...
def make_training_pair(
    ...
    **kwargs,
    ...
    filtered_downsampled = blurred[::ds_step, ::ds_step, ::ds_step, :]  # (nz, ny, nx, 3) -> (nz//ds_step, ny//ds_step, nx//ds_step, 3)
    truth_coarse = average_volumes(dns_velocity, ds_step)  # (nz, ny, nx, 3) -> (nz//ds_step, ny//ds_step, nx//ds_step, 3)
...
class ClosureCNNDataset(Dataset):
    ...
    def __getitem__(self, i: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        target_data = np.transpose(target_data, (3, 0, 1, 2))  # (coarse_nz, coarse_ny, coarse_nx, 3) -> (3, coarse_nz, coarse_ny, coarse_nx)
...
def build_inference_fn(model: nn.Module, sample_shape: tuple, device: torch.device):
    ...
    def predict(input_array: np.ndarray) -> np.ndarray:
        ...
        pred = model(x)[0].cpu().numpy()  # (1, 3, coarse_nz, coarse_ny, coarse_nx) -> (3, coarse_nz, coarse_ny, coarse_nx)
        return np.transpose(pred, (1, 2, 3, 0))  # (3, coarse_nz, coarse_ny, coarse_nx) -> (coarse_nz, coarse_ny, coarse_nx, 3)
    return predict
